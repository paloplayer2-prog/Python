...
def draw_checkerboard():
    for row in range(SCREEN_HEIGHT // SNAKE_SIZE):
        for col in range(SCREEN_WIDTH // SNAKE_SIZE):
...
def reset_game():
    global head_x, head_y, x_change, y_change, snake_body, snake_length, food_x, food_y, game_over, direction, next_direction
    head_x = 300
    head_y = 300
    x_change = 0
    y_change = 0
...
while running:
    ...
    if game_over:
        msg1 = game_over_font.render("GAME OVER", True, (200, 30, 30))
        msg2 = font.render("Press SPACE to Play Again", True, WHITE)
        screen.blit(msg1, [SCREEN_WIDTH // 2 - 120, SCREEN_HEIGHT // 2 - 40])
        screen.blit(msg2, [SCREEN_WIDTH // 2 - 140, SCREEN_HEIGHT // 2 + 15])
